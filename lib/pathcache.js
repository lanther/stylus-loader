...
function resolvers(options, webpackResolver) {
  ...
  var webpackResolve = function(context, path) {
    ...
    return whenWebpackResolver(context, path + '.styl')
      ...
      // If this fails, try the original path - e.g. if you did @import 'module/file'
      // then you might have configured 'module' as an alias in webpack, but this won't
      // work if you import it as './module/file'. Technically you should write '~module/file'
      // for this to work, but this fallback can be useful in certain circumstances
      .catch(function() { return whenWebpackResolver(context, originalPath + '.styl'); })
      .catch(function() { return whenWebpackResolver(context, originalPath); })
